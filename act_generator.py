...
def get_id(env, alg_id):
    ...
    if alg_id % env.pa.alg_num == 0:  #sjf + PK
        ...
        for j in xrange(env.mac_count):
            ...
            if env.check_act(act):
                ...
                for k in xrange(env.pa.res_num):
                    res_avail = (env.macs[j].state[k] == 0)
                    score += res_avail.sum() * env.jobs[idx].res_vec[k]
                    # score += env.jobs[i].res_vec[k] * 1.0 / res_avail.sum()
                if score > max_score:
                    max_score = score
                    ret_act = MOD * idx + j

    if alg_id % env.pa.alg_num == 1:  #sjf + ML
        min_duration = sys.maxint
        max_score = 0
        idx = 0
        for i in xrange(job_n):
        ...
        for j in xrange(env.mac_count):
            ...
            if env.check_act(act):
                ...
                for k in xrange(env.pa.res_num):
                    res_avail = (env.macs[j].state[k] == 0)
                    # score += res_avail.sum() * env.jobs[idx].res_vec[k]
                    score += 1 - (env.jobs[idx].res_vec[k] * 1.0 / res_avail.sum())
                if score > max_score:
                    max_score = score
                    ret_act = MOD * idx + j
    ...
    elif alg_id % env.pa.alg_num == 2:     #cp + pk
        max_len = 0
        max_score = 0
        idx = 0
        for i in xrange(job_n):
        ...
        for j in xrange(env.mac_count):
            ...
            if env.check_act(act):
                ...
                if score > max_score:
                    max_score = score
                    ret_act = MOD * idx + j
    elif alg_id % env.pa.alg_num == 3:     #cp + ML
        max_len = 0
        max_score = 0
        idx = 0
        for i in xrange(job_n):
